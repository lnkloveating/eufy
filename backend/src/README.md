...
- `eufy_security_agents/api`：REST、SSE、schema 与依赖注入
- `eufy_security_agents/application`：后续独立用例服务
- `eufy_security_agents/domain`：纯领域模型、策略、事件和端口
- `eufy_security_agents/agents`：预测、机会、产品、评审和产品定义Agent
- `eufy_security_agents/orchestration`：多 Agent 路由、图和执行状态
- `eufy_security_agents/infrastructure`：本地证据、SQLite与LLM适配器
